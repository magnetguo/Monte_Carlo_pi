...
int main(int argc, char* argv[])
{
	...
	req_s = malloc(sizeof(MPI_Request)*(p-1));
	req_r = malloc(sizeof(MPI_Request)*(p-1));
	...
	if (my_rank == 0)
	{
		...
		while(turn < TURNS){//每段都是一个循环
			...
			if (turn != 0)
				MPI_Waitall(p-1, req_s, MPI_STATUS_IGNORE);

			gettimeofday(&time6, NULL);
			fprintf(log_detailed, "before_Random\n");
            
            time_scatter += ((double)(time6.tv_sec*1000000+time4.tv_usec) - (double)(time3.tv_sec*1000000+time3.tv_usec)) / 1000;
			...
			{
				//if (i != 0) {
					MPI_Isend(&Sendbuffer[i*random_each], random_each, MPI_INT, i, i, MPI_COMM_WORLD, &req_s[i-1]);
					//MPI_Wait(&req_s[i-1], MPI_STATUS_IGNORE);
				//}
				//else
			...
			{
				x = (Sendbuffer[i] / (double) RAND_MAX)*2-1;
				y = (Sendbuffer[i+1] / (double) RAND_MAX)*2-1;
				if(x*x+y*y <= 1.0)
					number_in_circle++;
			}
	...
	else
	{
		...
		while(turn < TURNS)
		{
			...
			MPI_Irecv(Recvbuffer, random_each, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &req_s[my_rank-1]);
			//计算核心逻辑
			MPI_Wait(&req_s[my_rank-1], MPI_STATUS_IGNORE);
			for(long i=0; i<random_each; i+=2)
			{
			...
			//printf("the number in rank %d is %lu\n", my_rank, number_in_circle);
			turn++;
		}
		MPI_Reduce(&number_in_circle, &number_in_circles, 1, MPI_UNSIGNED_LONG, MPI_SUM, 0, MPI_COMM_WORLD);
			//printf("%ld\n",number_in_circle);
